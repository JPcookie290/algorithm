...
class Knight {
    ...
    knightMoves( start: number[], index: number[] ) {
        const coordinates: number[] = []
        const moveCoord: number[] = index;

        if (!this.checkCoordinates(start)) return null;
        coordinates.push(start[0] + moveCoord[0]);
        coordinates.push(start[1] + moveCoord[1]);
        if (!this.checkCoordinates(coordinates)) return null;
        
        return coordinates
    }

    bdf(start: number[], end: number[]) {
